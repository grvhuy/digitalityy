...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromCheckout(state, action: PayloadAction<{ id: string }>) {
      state.checkoutItems = state.checkoutItems.filter(item => item._id !== action.payload.id)
    },
...
export const { addToCart, removeFromCart, clearCart, addToCheckout, removeFromCheckout, increment, decrement } = cartSlice.actions
